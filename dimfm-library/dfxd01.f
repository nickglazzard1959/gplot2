      SUBROUTINE DFXD01(IF,XARG,YARG,ZARG,NARG)
C --- -----------------------------------------------------------------
C
C --- DFDX01: VECTOR OUTPUT DRIVER FOR BINARY EPS OUTPUT.
C
...
C NG- FOR WORKING LINE WIDTH CONTROL
      ...
      SAVE FMWLST
C
      REAL XDLAST, YDLAST, SSLAST
      SAVE XDLAST, YDLAST, SSLAST
C
      REAL DSCALE, XDCO, YDCO
      SAVE DSCALE, XDCO, YDCO, DRAW, RGB0, IFLAST
      SAVE RASTEX, NRAST2, NRAST1, HESDC, VESDC
      SAVE XDCOR0, YDCOR0
...
C
      DATA FMWLST/-1.0/
      DATA XDLAST/0.0/, YDLAST/0.0/, SSLAST/0.0/
      DATA DSCALE/1.0/, XDCO/0.0/, YDCO/0.0/
      DATA DRAW/0/, RGB0/.FALSE./, IFLAST/0/
      DATA RASTEX/.FALSE./, NRAST2/0/, NRAST1/0/
...
C   NG- NOTE: THIS IS ACTUALLY FOR A *BINARY* DEVICE.
         Z = ZINT(II) * ZRGB(II)
         IF( Z .GT. 0.001 )THEN
            IZ = 1
         ELSE
...
C    DFX..F USED TO HANDLE RASTER OUTPUT FOR DEVICE
      CALL DFX01F(XARG,YARG,ZARG,NARG,IPAR(1),IPAR(2),DEVLUT,MAXLUT,
     1    LCOL,BLANK,WSBL)
