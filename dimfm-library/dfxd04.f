      SUBROUTINE DFXD04(IF,XARG,YARG,ZARG,NARG)
C --- -----------------------------------------------------------------
C
C --- DFDX04: VECTOR OUTPUT DRIVER FOR TEK4K OUTPUT.
C
...
C
      ...
      PARAMETER( XDCMAX=1023.0, YDCMAX=767.0, XDCORG=0.0, YDCORG=0.0 )
C
...
C   ****************************************************
C
C DO NOTHING FOR TEK4K - BEAM IS EITHER ON OR OFF.
C AND IT HAS NO SETTABLE COLOUR.
...
C     DFX..F USED TO HANDLE RASTER OUTPUT FOR DEVICE
      CALL DFX04F(XARG,YARG,ZARG,NARG,IPAR(1),IPAR(2),DEVLUT,MAXLUT,
     1     LCOL,BLANK,WSBL)
...
C      CALL TXBEGIN
C      PRINT *,'XDCO, YDCO',XDCO,YDCO,XDCOR0, YDCOR0
C      CALL TXEND
