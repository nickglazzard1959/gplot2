...
C    DIMFILM CURRENTLY SUPPORTS MAXBET UP TO A MAXIMUM OF 3
      ...
      INCLUDE 'dfxc05.cmn'
      INCLUDE 'dfxc12.cmn'
C    MAXLEV IS NUMBER OF SUPER-/SUB-SCRIPT LEVELS
C           BASE IS LEVEL 0
C    MAXFRC IS NUMBER OF NESTED FRACTIONS
C           BASE IS FRACTION 0
      PARAMETER (MAXLEV=6,MAXFRC=2)
...
C
      ...
      CHARACTER*5 ABET
      CHARACTER*6 SM(0:2)
      SAVE IEPT1, IEPT2, IEPT3, ESC1, ESC2, ESCLST, ABET, SM, IMS
...
C
   ...
   13 BACK = .FALSE.
    ...
    9 CONTINUE
C
C NG- OCT 2008: CX IS SOMETIMES INDEFINITE AT THIS POINT.
C THIS MAY BE DUE TO INADEQUACIES IN THE FONT DEFINITIONS
C WE ARE NOW USING, BUT I HAVE NOT FOUND THE REAL ANSWER.
C IT IS FUDGED HORRIBLY IN THE OUTPUT DRIVER.
C CDC EMULATION ONLY.
C
C      PRINT 9876,CX
C 9876 FORMAT(1X,'CX=',F12.6,' BEF DFX202().')
C
      CALL DFX202(NCHAR,L1,L2)
C    DO INDIVIDUAL CHARACTER
C    ACCUMULATE MINIMUM/MAXIMUM VERTICAL EXTENT
C    (REFLECT ACTUAL CHAR SIZES)
      IB1 = INT(LYTENT(NCHAR,NBET)*CYF)
      IB2 = INT(UYTENT(NCHAR,NBET)*CYF)
