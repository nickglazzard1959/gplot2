...
C --- -----------------------------------------------------------------
C
C --- THIS IS THE DEVICE SELECTION ROUTINE FOR COLOUR EPS OUTPUT.
...
C
      ...
      INCLUDE 'dfxcbd.cmn'
C
C --- DECLARE THE ACTION ROUTINE DFXD02 TO BE EXTERNAL, SO WE CAN
...
C
      CALL DFX5AA( 02, DCXSIZ, DCYSIZ )
C      PRINT 100,7
      CALL DFXG55( 02, 0., DCXSIZ, 0., DCYSIZ )
C      PRINT 100,8
      DCXFR = 1.0
      DCYFR = DCYSIZ / DCXSIZ   
C      PRINT 100,10
      CALL DFXG54( 02, 0., DCXFR, 0., DCYFR )
C      PRINT 100,11
C
...
C
      IF( ROUTIN .EQ. 'DEPCOL' ) ROUTIN = STARS6
C
C      PRINT 100,20
      RETURN
      END
