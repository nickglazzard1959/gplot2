...
where it behaves identically to the NOS version.
\newpara
...
\newpara
\textbf{GPLOT} is currently at version 0.6. The first usable version (0.1) could not output EPSF files as such, but used a
limited character set format that could be trivially translated to EPSF. V0.2 improved Tektronix 401x support. V0.58
added significant -- if idiosyncratic -- facilities for function evaluation and other programmability features, as well as SVG
output. It also exposed a lot more of DIMFILM's capabilities. Most of what can be done with DIMFILM using a specially
written Fortran program can now be done from \textbf{GPLOT}. V0.59 fixed some bugs in parameter substitution. V0.6 tidied
up a few things and supported building on ``Unix-like'' systems as well as NOS.
\newpara
...
\newpara
\textbf{GPLOT} V0.1 was written in late 2013. A `package' that might be installable fairly easily was put together in March 2015. Improvements
to Tektronix support were made in 2022 and major enhancements were made in late 2022 to early 2023. Some bug fixes and a
documentation update were done in May 2024. V0.6 was completed in July 2025.
...
That does not mean that lower case text cannot be plotted -- it can.

\newpara
On ``Unix-like'' systems, all lower case input is converted to upper case. That is, \textbf{GPLOT}
behaves exactly as it does on NOS.
\end{quote}
...
\item \textttc{DEVICE NAME [OUTPUT-FILE]} --- Select the output device.\\
   ...
   \texttt{SAVE} or \texttt{REPLACE}. See Section \ref{devs} for the available devices and their
   characteristics.
...
\item \textttc{COLOUR R G B} --- Set the RGB colour to use for drawing.\\
   All lines (and all the graphical elements are lines) will be drawn in this
   colour.
\item \textttc{WIDTH WIDTH} --- Set the line width.\\
   The width is a multiple of a device dependent "base width" and not all devices
   can draw wide lines (currently, only the Tektronix device cannot).
...
\subsection{Graph Annotation Commands}\label{devs}
...
\item \textttc{GTERM} --- \textbf{GTerm} colour graphics terminal.\\
   ...
   anti-aliasing and anything displayed can be saved as SVG. \textbf{GTerm} runs on Linux and macOS, but not
...
\item \textttc{SVG NAME} --- Output SVG (Scalable Vector Graphics) to a file. This can be included in HTML files, and
	this format is used by the \texttt{PLNOTES} application to insert graphs and graphics into documents. 
\end{itemize}
\newpara
