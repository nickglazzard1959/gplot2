...
\section{Using GPLOT from Python programs on Unix-like systems}
\newpara
Plotting data from inside a Python program is often useful (pretty obviously!).
The package universally used for this is the excellent \textbf{matplotlib} but a small
Python library can be used to plot data with \textbf{GPLOT} too.
...
programs is highly recommended. These tools
must also be installed, as they include viewers for SVG and EPS files which the
library will potentially try to run.

\newpara
After installation, you can \texttt{import gplot} and use the \texttt{gplot} class to provide
access to the graph plotting subset of \textbf{GPLOT} commands.

\newpara
Note that behind the scenes, this works in the simplest way possible: a list of \textbf{GPLOT}
commands is compiled, and the \textbf{GPLOT} program is run to execute them, generating (usually)
an SVG format file which can be displayed (with \texttt{svgview}) or renamed to preserve the
output file.

\newpara
Although this is a simple approach, it is quite effective on modern hardware (\textbf{GPLOT/DIMFILM}
is very lightweight by today's standards) and is not slow. In fact, the Python based \texttt{svgview}
program is often significantly slower to run than \textbf{GPLOT}, although the combination is quite
fast enough for normal purposes.

\newpara
A minimal example of the use of the \texttt{gplot} module is:
\begin{lstlisting}
import gplot
import numpy as np
...
\newpara
This will display a cycle (and bit) of a sine wave in a pop-up window when \texttt{draw()} is called.
...
\newpara
The default output device is \texttt{svg}. Any \textbf{GPLOT} device can be used,
including \texttt{epscol} for EPS format files. In this case,
the \texttt{offset=(0,0)} option to the constructor might be useful to remove or adjust the default
margin of half an inch (note that sizes and offsets are in inches for EPS).

\newpara
The names of the \texttt{gplot} class functions are usually identical to the \textbf{GPLOT} command names they generate
and the arguments, when they are names, also follow the names used in \textbf{GPLOT}, so the ``cheat sheet'' and other
\textbf{GPLOT} documentation can be used directly for this Python interface.

\newpara
There are a few exceptions:
\begin{itemize}
\item When \texttt{on} and \texttt{off} values are concerned, these can be given as strings, or
\texttt{"yes"} and \texttt{"no"} can be used or \texttt{True} and \texttt{False} can be used.
\item  Unlike \textbf{GPLOT} commands, Python function names cannot be abbreviated (!), so
\texttt{csgroup} has been shortened to \texttt{csg}. 
\item  There are three separate functions to plot data with error bars (as the mechanism \textbf{GPLOT} uses to distinguish
these types isn't applicable to Python). These are: \texttt{xyplot\_yerrors()},
\texttt{xyplot\_yerrors\_asym()} and \texttt{xyplot\_xyerrors()}.
These can be given a \texttt{cmd} argument of \texttt{xyline} or \texttt{xypoint} to control the type of
  plot to generate.
\item  The marker to be used for points can be specified with a single character string or with a number. This
  may make selecting commonly used markers simpler.
\item  If drawing a single graph (without sub-figures), it isn't necessary to use \texttt{key()} to draw the key.
  This \emph{is}
  needed, though, for all sub-figures except the last if a plot with multiple sub-figures and keys is being
  made.
\item There is an escape mechanism that allows any \textbf{GPLOT} command to be inserted in to the command stream
  executed when \texttt{draw()} is called: \texttt{command("command text")}. This can be used to run the evaluator,
  for example.
\end{itemize}

\newpara
Speaking of exceptions, the \texttt{gplot} module will throw exceptions to signal error conditions by default. It is 
possible to choose boolean condition code returns instead by setting \texttt{throwup=False} in the constructor. The
exceptions thrown are:
\begin{itemize}
\item \texttt{ValueError} when improper arguments are supplied to a function.
\item \texttt{RuntimeError} if there is any problem related to running \textbf{GPLOT} or a viewing program, or if the
  constructor failed (which will throw \texttt{ValueError}) and the user has insisted on continuing anyway.
\end{itemize}

\newpara
The subset of tutorial examples that cover graph plotting have been re-implemented in Python
in the file: \texttt{python/testpyif.py} to serve as examples and also to test almost all features
of the \texttt{gplot} module.
...
\subsection{Automated documentation for the \texttt{gplot} class}

\newpara
Automatic documentation for the \texttt{gplot} class was created with:
\begin{lstlisting}
pydoc2md gplot . 
\end{lstlisting}
The result is pretty basic, but hopefully will answer any detailed questions.

\begin{itemize}
 
\item{\textbf{gplot} class}\\
Provide a Python callable interface to \textbf{GPLOT}.
...
\end{lstlisting}
Create a \texttt{gplot} class instance.

\item{\texttt{csg}}\\
\begin{lstlisting}
def csg(self, which):
\end{lstlisting}
Choose the colour/style group to which the following \texttt{colour()}, \texttt{width()} apply.
...
\end{lstlisting}
Draw \texttt{xyline}, \texttt{xypoint} or \texttt{xyhistogram}.

\item{\texttt{xyplot\_yerrors}}\\
\begin{lstlisting}
def xyplot_yerrors(self, x, y, e, cmd):
\end{lstlisting}
Draw \texttt{xyline}, \texttt{xypoint} with symmetric Y error bars

\item{\texttt{xyplot\_yerrors\_asym}}\\
\begin{lstlisting}
def xyplot_yerrors_asym(self, x, y, e1, e2, cmd):
\end{lstlisting}
Draw \texttt{xyline}, \texttt{xypoint} with asymmetric Y error bars

\item{\texttt{xyplot\_xyerrors}}\\
\begin{lstlisting}
def xyplot_xyerrors(self, x, y, ex, ey, cmd):
\end{lstlisting}
Draw \texttt{xyline}, \texttt{xypoint} with symmetric X and Y error bars
...
def grmove(self, x, y):
\end{lstlisting}
Move to graph coordinates \texttt{(x,y)}

\item{\texttt{grdraw}}\\
\begin{lstlisting}
def grdraw(self, x, y):
\end{lstlisting}
Draw to graph coordinates \texttt{(x,y)}
...
\end{lstlisting}
  ...
  \texttt{axisname}: \texttt{none, x, y} or \texttt{both}
...
\end{lstlisting}
  ...
  \texttt{objname}: \texttt{pane, blank, bounds, device}
...
\end{lstlisting}
  ...
  \texttt{interpname}: \texttt{linear, cubic, quintic}

\item{\texttt{histstyle}}\\
\begin{lstlisting}
def histstyle(self, histmode, width=None):
\end{lstlisting}
  Set histogram drawing style.\\
  \texttt{histmode}: \texttt{abut, abut+shade, lines, wide, wide+shade}
...
\end{lstlisting}
  ...
  \texttt{stylename}: \texttt{boxed, axes, open}
...
\end{lstlisting}
Add a key for the last specified \texttt{xyline} or \texttt{xypoint}.

\item{\texttt{graphmode}}\\
\begin{lstlisting}
...
\end{lstlisting}
Draw the next graph as a sub-figure, \texttt{(ix,iy)} in a grid of nx x ny.
...
\end{lstlisting}
  ...
  \texttt{(gx,gy)} are graph coordinates to which the arrow points.\\
  \texttt{length} is the length of the arrow shaft in bounds units.\\
  \texttt{angle} is the angle of the arrow shaft in degrees wrt +X.

\item{\texttt{command}}\\
\begin{lstlisting}
def command(self, cmd):
\end{lstlisting}
Add an arbitrary \textbf{GPLOT} command. Useful for tests.
