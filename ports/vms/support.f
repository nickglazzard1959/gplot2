...
C      LOGICAL FUNCTION IBTEST(IV, IB)
C----
C     TEST BIT IB IN INTEGER IV
C----      
C      INTEGER IV, IB
C      IBTEST = BTEST(IV, IB)
C      RETURN
C      END

      SUBROUTINE CONNEC( I )
C----
C     STUB FOR NOS CONNEC SUBROUTINE.
C     ON VMS, ASSIGN A CHANNEL TO SYS$OUTPUT SO WE CAN DO RAW IO TO IT.
C----
      INTEGER STATUS, SYS$ASSIGN
      INTEGER*2 OUT_CHAN
      COMMON /VMSIO/ OUT_CHAN
C
      STATUS = SYS$ASSIGN('SYS$OUTPUT', OUT_CHAN,,)
      IF (.NOT. STATUS) THEN
         WRITE (*,*) 'CONNEC: SYS$ASSIGN failed, status = ', STATUS
      ...
      SUBROUTINE DISCON( I )
C----
C     STUB FOR NOS DISCON SUBROUTINE.
C     ON VMS, DEASSIGN THE CHANNEL ASSIGNED IN CONNEC.
C----
      INTEGER STATUS, SYS$DASSGN
      INTEGER*2 OUT_CHAN
      COMMON /VMSIO/ OUT_CHAN
C
      STATUS = SYS$DASSGN(%VAL(OUT_CHAN))
      IF (.NOT. STATUS) THEN
         WRITE (*,*) 'DISCON: SYS$DASSGN failed, status = ', STATUS,
     +        '(ignored).'
      ENDIF       
      RETURN
      END

      SUBROUTINE SNDBYT( BDATA, NDATA )
C----
C     SEND NDATA RAW BYTES FROM BDATA TO A TERMINAL.
C----
      INTEGER NDATA
      BYTE BDATA(NDATA)
C
...
C
     ...
     +     ,,,)
      ...
      IF (.NOT. STATUS) THEN
         WRITE (*,*) 'SYS$QIOW queue failed, status = ', STATUS
         STOP 'CANNOT CONTINUE.'
      ENDIF
      RETURN
      END
...
C----
C     RETURN NOS ORIGIN TYPE. ALWAYS INTERACTIVE (3) ON VMS.
C----
      IGETOT = 3
      RETURN
      END
...
C----
C     RETURN ASCII CHARACTER CODE FOR CHARACTER C.
C     SAME AS ICHAR() ON VMS.
C----
      CHARACTER*1 C
      IACHAR = ICHAR(C)
      RETURN
...
C
      ...
      INTEGER ICARG, STATUS
      INTEGER GETWORD
      LOGICAL FIRST
      EXTERNAL LIB$GET_FOREIGN
      SAVE ICARG, FIRST, STATUS
      DATA ICARG/1/, FIRST/.TRUE./
C
      KEY = ' '
      VALUE = ' '
      ISTAT = 1
      ...
      IF( FIRST )THEN
         ...
         IF( STATUS .NE. 1 )THEN
            WRITE(*,*) 'WARNING: Failed to obtain command line.'
         ENDIF
         FIRST = .FALSE.
      ENDIF
      IF( STATUS .NE. 1 )THEN
         ...
         RETURN
      ENDIF
      IF( GETWORD( ICARG, CMDLINE, INPARM ) .LT. 0 )THEN
         ISTAT = -1
         RETURN
      ENDIF
      ICARG = ICARG + 1
